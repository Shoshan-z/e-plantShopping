...
function ProductList() {
  ...
  function handleAddToCart(plant) {
    ...
    setAddedToCart((prevState) => {return {...prevState, [plant.name]: true}}); 
    ...
    <div>
      ...
      {!showCart ? (
        <div className="product-grid">
          {plantsArray.map((itm, idx) => {
            return (
              <div key={idx}>
                <h1><div>{itm.category}</div></h1>
                <div className="product-list">
                  {itm.plants.map((innerItm, innerIdx) => {
                    return(
                      <div className="product-card" key={innerIdx}>
                        <img className="product-image" src={innerItm.image} alt={innerItm.name}></img>
